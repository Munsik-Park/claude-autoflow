...
- **Option B.2 — context discriminator field (`context: meta|instance`) in evaluation JSON.** Rejected because the per-issue `weights.json` plus `AUTO_FAIL_KEY` env override already provides the per-context discrimination machinery (`AUTO_FAIL_KEY` defaulting at check-autoflow-gate.sh:101; per-category weight lookup against `weights.json` at check-autoflow-gate.sh:265-273). Adding a schema field duplicates that mechanism and forces every evaluator to declare context, expanding the JSON contract for no new behaviour.
- **Embedding Meta-context categories (Consistency / Documentation) in CLAUDE.md.template *without* a documented Instance customisation path.** Rejected because that variant would propagate the Meta defaults into user instances silently — readers of the template would see Consistency/Documentation and have no in-file signal that Security/Performance + `AUTO_FAIL_KEY=security` is the typical Instance choice. Resolved by shipping the Meta-context categories as the template's project-default *with* an in-file HTML comment block (CLAUDE.md.template:445-452) that documents the swap to Security/Performance and the `AUTO_FAIL_KEY=security` override, plus a user-visible cross-link to docs/evaluation-system.md > Meta vs. Instance Context. The HTML comment is invisible in rendered output but visible in template source, so a contributor editing the template sees the customisation path before adopting the defaults.

**What this means**

Three concrete invariants for future contributors: (a) CLAUDE.md and CLAUDE.md.template ship the same Meta-context category set, and the template's HTML comment block (CLAUDE.md.template:445-452) is the canonical place where the Instance customisation path is documented — modifications to the template categories must keep the comment block in sync; (b) the hook never gains hard-coded category names — runtime discovery and `AUTO_FAIL_KEY` are the only customisation points; (c) any proposal to revisit the single-shape flattening must first cite the deferred follow-up issue and add to its evidence rather than re-opening the question here.
